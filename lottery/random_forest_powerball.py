...
def main():
    data_df = pd.read_csv('resource/preprocess-data-powerball.csv')
    ...
    labels = np.array(data_df['result'])
    # labels = np.mod(labels, 2)
    # labels = np.floor_divide(labels, 11)
    print(labels)
    ...
    # # print(features)
    print(time.time()%314)
    train_features, test_features, train_labels, test_labels = train_test_split(features, labels, test_size = 0.25, random_state = int(time.time()%314))
    ...
    baseline_preds = np.random.randint(2, size=test_labels.shape)
    print(baseline_preds)
    # baseline_preds = test_features[:, feature_list.index('average')]
    # # Baseline errors, and display average baseline error
    baseline_errors = abs(baseline_preds - test_labels)
    print(baseline_errors)
    print(list(baseline_errors).count(0))
    print(list(baseline_errors).count(1))
    print(list(baseline_errors).count(2))
    ...
    print(errors)
    print(list(errors).count(0))
    print(list(errors).count(1))
    print(list(errors).count(2))
